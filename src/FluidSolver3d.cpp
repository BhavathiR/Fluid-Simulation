...
void FluidSolver3D::labelGrid() {
	...
	for (int i = 0; i < m_particles->size(); i++) {
		...
		int cell[3];
		getGridCellIndex(cell, m_particles->at(i).pos, m_dx);
...
void FluidSolver3D::particlesToGrid() {
	...
	for (int p = 0; p < m_particles->size(); p++) {
		...
		int ind[3];
		getGridCellIndex(ind,curParticle.pos, m_dx);
		int i0 = ind[0];
		int j0 = ind[1];
		int k0 = ind[2];
...
void FluidSolver3D::advectParticles(int C) {
	for (int i = 0; i < m_particles->size(); i++) {
		...
		while (!finished) {
			...
			int cell[3];
			getGridCellIndex(cell,curParticle->pos, m_dx);
			int j = cell[0];
			int k = cell[1];
			int l = cell[2];
...
void FluidSolver3D::cleanupParticles(float dx) {
	...
	while(!finished && m_particles->size() > 0) {
		int ind[3];
		getGridCellIndex(ind, m_particles->at(i).pos, m_dx);
...
Vec3 FluidSolver3D::interpVel(SimUtil::Mat3Df &uGrid, SimUtil::Mat3Df &vGrid, SimUtil::Mat3Df &wGrid, Vec3 pos) {
	...
	int cell[3];
	getGridCellIndex(cell,pos, m_dx);
	int i = cell[0];
	int j = cell[1];
	int k = cell[2];
...
bool FluidSolver3D::projectParticle(Particle3D *particle, float dx) {
	...
	int index[3];
	getGridCellIndex(index,particle->pos, m_dx);
